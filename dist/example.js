...
function runExampleCombat() {
    ...
    const controller = new CombatController(initialState, COMBO_DATABASE, STATUS_DATABASE, ENEMY_ACTION_DATABASE, FALLBACK_ACTION);
    console.log('Player HP:', controller.getState().player.currentHp);
    console.log('Enemies:', controller.getState().enemies.map(e => `${e.name} (${e.currentHp}/${e.maxHp} HP)`).join(', '));
    console.log('\n=== TURN 1 ===\n');
    ...
    console.log('Enemies:', state.enemies.map(e => `${e.name} (${e.currentHp}/${e.maxHp} HP, ${e.statuses.length} statuses)`).join(', '));
    ...
    console.log('\nPlayer HP:', state.player.currentHp);
    console.log('Enemies:', state.enemies.map(e => `${e.name} (${e.currentHp}/${e.maxHp} HP, alive: ${e.isAlive})`).join(', '));
...
function testComboParser() {
    ...
    const controller = new CombatController(createInitialState(), COMBO_DATABASE, STATUS_DATABASE, ENEMY_ACTION_DATABASE, FALLBACK_ACTION);
    ...
    testSequences.forEach(sequence => {
        ...
        sequence.forEach(dir => controller.addInput(dir));
